...
const ProductDetails = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    const colorGroup = product.images.find(
  ...
  const handleAddToCart = () => {
    ...
    alert(
      `Added to cart: ${product.name} - ${selectedColor} - ${selectedSize} - Quantity: ${quantity}`,
    );
  };
  ...
  const handleBuyNow = () => {
    ...
    const orderItem = {
      ...
      price: product.price,
      imageUrl:
        selectedImages.length > 0 ? bufferToBase64(selectedImages[0]) : "",
      color: selectedColor,
      size: selectedSize,
      quantity: quantity,
      stock: product.stock,
