...
import {
  getProducts,
  deleteProduct,
} from "../../components/EcommerceAdmin/productService";
...
const EcomProductsView = () => {
  ...
  const fetchAndSetProducts = useCallback(async () => {
    try {
      ...
      setError(
        "Failed to connect to the server. Please ensure the backend is running and check your network connection.",
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    if (productLineFilter !== "all") {
      productsToFilter = productsToFilter.filter(
        (p) => p.productLine === productLineFilter,
      );
    }
    if (genderFilter !== "all") {
      productsToFilter = productsToFilter.filter(
    ...
    if (searchQuery) {
      ...
      productsToFilter = productsToFilter.filter(
        (p) =>
          ...
          (p.material?.toLowerCase() ?? "").includes(lowercasedQuery) ||
  ...
  if (loading)
    return (
      <p style={{ color: "white", padding: "2rem" }}>Loading Products...</p>
    );
  // This error message will now appear if the network connection fails
  if (error)
    return (
      <p style={{ color: "#f87171", padding: "2rem", fontSize: "1.2rem" }}>
        {error}
      </p>
    );
