...
const EditProductForm = () => {
  ...
  const handleColorSelect = (color) => {
    if (color && !formData.colors.includes(color)) {
      setFormData((prev) => ({
        ...
        colorImages: { ...prev.colorImages, [color]: [] },
      }));
    }
    setSelectedColor("");
  };
  ...
  const handleRemoveImage = useCallback((fileToRemove, color) => {
    setFormData((prev) => ({
      ...
      colorImages: {
        ...prev.colorImages,
        [color]: prev.colorImages[color].filter(
  ...
  const handleAddSize = () => {
    if (newSize && !formData.sizes.includes(newSize)) {
      setFormData((p) => ({ ...p, sizes: [...p.sizes, newSize] }));
      setNewSize("");
    }
  };
  ...
  const handleRemoveColor = (color) =>
    setFormData((p) => {
      const newColors = p.colors.filter((c) => c !== color);
      const newImages = { ...p.colorImages };
      delete newImages[color];
      return { ...p, colors: newColors, colorImages: newImages };
    });
  const handleRemoveSize = (size) =>
    setFormData((p) => ({ ...p, sizes: p.sizes.filter((s) => s !== size) }));
      ...
      >
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="color-images-section">
            {formData.colors.map((c) => (
              <div key={c} className="color-image-upload">
                <h4>Images for {c}</h4>
                <FileUploadBox
                  color={c}
