...
const MailContentManager = () => {
  ...
  const flattenedItems = useMemo(() => {
    ...
    const allItems = Object.entries(newsletterContent.categories).flatMap(
      ([categoryId, items]) =>
        items.map((item) => ({
          ...item,
          categoryId,
  ...
  const handleDelete = (itemToDelete) => {
    showConfirmation(
      `Delete "${itemToDelete.caption || itemToDelete.data}" permanently?`,
      async () => {
        const updatedContent = JSON.parse(JSON.stringify(newsletterContent));
        const filtered = updatedContent.categories[
          itemToDelete.categoryId
        ].filter((i) => i._id !== itemToDelete._id);
  ...
  return (
    <div className="list-page-container">
      <header className="list-page-header">
        ...
        <Link to="/admin/mail/add" className="create-btn">
              ...
              {flattenedItems.map((item) => (
                <tr key={item._id}>
                  <td className="title-cell">
                    {item.caption || item.data || "(No Caption)"}
                  </td>
...
const WebsiteArticleList = () => {
  ...
  const filteredArticles = useMemo(() => {
    return articles.filter((article) => {
      const locationMatch =
        locationFilter === "All" || article.location === locationFilter;
      const categoryMatch =
  ...
  return (
    <div className="list-page-container">
      <header className="list-page-header">
        ...
        <Link to={`/admin/editor/${section}`} className="create-btn">
              ...
              {filteredArticles.map((article) => (
                <tr key={article._id}>
                  ...
                  <td>
                    ...
                    <span
                      className={`status-pill ${typeof article.status === "string" ? article.status.toLowerCase() : "published"}`}
                    >
                  ...
                  <td className="action-cell">
                    ...
                    <button
                      onClick={() =>
                        navigate(`/admin/editor/${section}/${article.slug}`)
                      }
                      className="action-icon edit"
                      title="Edit"
                    >
